...
COPY ./src/cart/cart.csproj ./src/cart/
COPY ./NuGet.config ./

# Restore dependencies for the target architecture
RUN dotnet restore ./src/cart/cart.csproj -r linux-musl-$TARGETARCH
...
RUN dotnet publish ./src/cart/cart.csproj -r linux-musl-$TARGETARCH --no-restore -o /cart
