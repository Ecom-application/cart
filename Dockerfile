...
COPY ./src/cartstore/cart.csproj ./src/
COPY ./NuGet.config ./

# Restore dependencies for the target architecture
RUN dotnet restore ./src/cartstore/cart.csproj -r linux-musl-$TARGETARCH
...
RUN dotnet publish ./src/cartstore/cart.csproj -r linux-musl-$TARGETARCH --no-restore -o /cart
